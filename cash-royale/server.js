...
app.post('/api/create-pix', async (req, res) => {
  try {
    ...
    await db.dbSavePayment(paymentId, { userId, packId, coins: pack.coins });
    res.json({
      paymentId,
...
// -------------------------------------------------------
// 5) Gastar moedas (entrada da arena e outros)
// -------------------------------------------------------
...
app.post('/api/arena/join', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao entrar na arena', detail: err.message });
  }
});
...
app.post('/api/arena/leave', async (req, res) => {
  try {
    ...
    res.status(500).json({ error: 'Erro ao sair', detail: err.message });
  }
});
...
db.initDb().then(() => {
  arena.startLoop(db);
  app.listen(PORT, () => {
    console.log(`✅ Cash Royale rodando em http://localhost:${PORT}`);
    ...
    console.log(`   Entrada na arena: ${arena.CFG.fee} moedas | máx ${arena.CFG.maxPlayers} jogadores`);
